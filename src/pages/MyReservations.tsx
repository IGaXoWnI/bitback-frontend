...
interface ApiReservation {
  ...
  quantity?: number;
  pickup_code?: string;
  status: 'reserved' | 'picked_up' | 'canceled'; // API values
  created_at: string;
  updated_at: string;
  box: {
    id: number;
...
// Define our UI representation with mapped status values
interface Reservation extends Omit<ApiReservation, 'status'> {
  status: 'pending' | 'confirmed' | 'completed' | 'cancelled'; // UI values
}
...
const MyReservations: React.FC = () => {
    ...
    const fetchReservations = async () => {
      ...
      try {
        ...
        if (response.data.success && response.data.data && response.data.data.data) {
          ...
          const reservationsData = response.data.data.data as ApiReservation[];
          
          // Map API statuses to UI statuses
          const mappedData = reservationsData.map((item: ApiReservation): Reservation => ({
            ...item,
            status: item.status === 'reserved' ? 'pending' : 
          ...
          console.log("Loaded reservations:", mappedData.length);
        } else {
  ...
  const groupedReservations = {
    // Only 'pending' is considered upcoming since we've mapped 'reserved' to 'pending'
    upcoming: filteredReservations.filter(r => r.status === 'pending'),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* Filter tabs - Simplified to 3 options */}
                        ...
                        <div className="p-6 flex-1">
                          ...
                          <div className="mt-4 flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-2">
                              {reservation.status === 'confirmed' && reservation.pickup_code && (
                          ...
                          <div className="mt-4 flex items-center justify-between">
                            ...
                            {reservation.status === 'completed' && (
                              <button 
                                ...
                                className="bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-1.5 px-4 rounded-lg transition-colors flex items-center"
                              >
      ...
      {showReviewModal && selectedReservation && (
        <ReviewModal
          ...
          boxTitle={selectedReservation.box.title}
          businessName={selectedReservation.box.business.business_name}
          imageUrl={selectedReservation.box.image}
        />
      )}
    </div>
  );
